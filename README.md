...
- **剪贴板历史回溯**：自动检索最近 1 分钟内复制过的句子，找到包含该词的那句作为语境，无需手动操作
...
- **测完立刻生效**：查词时不再只给中文——先展示「适合你词汇水平的简单英语释义」（你会的词解释生词，而不是死记中文意思）
  - **LLM 模式**：自动按你的词汇水平约束用词（COBUILD 风格一句话释义）
  - **离线模式**：展示英英释义，超出你水平的词标为虚线高亮，点击即可再查
...
|---|---|
| **查单词/短语** | 选中文本 → 按 `Alt+Q` |
| **查词带语境** | 先 `Ctrl+C` 复制一句**包含该词**的句子（1 分钟内）→ 再选中该词按 `Alt+Q`，语境自动出现 |
