...
- 复习（语境挖空 + 英英释义优先，与 PC 完全同一条 SM-2 曲线）、查词、词表浏览/删除
- 查词四级兜底：**离线 mini 词典**（`dict/mini.json`，Wi-Fi 下首次加载后本会话内秒查）→ **PC 完整词典**（`/api/lookup`，含按词汇水平的英英释义 + LLM 简单释义）→ **直连 MyMemory 在线翻译**（免费无需配对，外出/未配对时的兜底）→ 如实提示未查到
- **可以先只用手机**：查词、入库、复习全程本地；首次配对时手机会把积累的生词与复习进度推上 PC（同步是双向的）
